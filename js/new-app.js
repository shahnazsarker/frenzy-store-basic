...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image mx-auto my-3" src=${product.image}></img>
    <div class="card-body text-center">
    <h3 class="card-title">${product.title}</h3>
    <p>Category: ${product.category}</p>
      <p class="card-text fas fa-star text-warning px-2"><span class="text-dark"> ${product.rating.rate} </span></p>
      <p class="card-text far fa-thumbs-up text-primary px-2"><span class="text-dark"> ${product.rating.count} </span></p>
      <h4 class="card-text">Price: <span class="text-success"> $ ${product.price}</span></h4>
      ...
      <button id="details-btn" class="btn btn-outline-danger"> Details <i class="fas fa-info-circle"></i></button>
      </div>
      </div> `;
    document.getElementById("all-products").appendChild(div);
  }
};
// item count and price calculation
let count = 0;
